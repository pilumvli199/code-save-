...
FIXED: is_market_open() takes current_time parameter
""" 
import pytz 
...
def is_market_open(current_time=None): 
    """
    ✅ FIXED: Check if market is open (9:15 AM - 3:30 PM)
    
    Args:
        current_time: Optional time object. If None, uses current time.
...
def get_market_state(current_time=None):
    """
    ✅ FIXED: Get current market state
    
    Args:
        current_time: Optional time object
    
    Returns:
        str: Market state (PREMARKET, OPEN, CLOSED, etc.)
    """
    if current_time is None:
        current_time = get_ist_time().time()
...
def get_market_status(): 
    """Get current market status with message""" 
    now = get_ist_time() 
...
def validate_strike_data(strike_data, min_strikes=5): 
    """ 
    ✅ UPDATED: Validate option chain data - need at least 5 strikes
    (Changed from 7 to 5 for analysis range compatibility)
    """ 
